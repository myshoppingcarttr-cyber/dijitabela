...
(function () {
  ...
  function giris(adim, msg) {
    R.innerHTML = (API.mode === "demo" ? '<div class="demo">Demo modu: herhangi bir e-posta, kod <b>123456</b>.</div>' : "") +
      ...
      '</form><button class="btn btn-p" id="gb">' + (adim === "kod" ? "Giriş" : "Kod gönder") + "</button></div>";
  ...
  function yukle() { return API.tumTeklifler().then(function (l) { S.list = l || []; ciz(); }); }
  ...
  function ciz() {
    ...
    R.innerHTML = (API.mode === "demo" ? '<div class="demo">Demo modu · <button class="btn btn-o sm" id="rs">Demo verilerini sil</button></div>' : "") +
      '<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:18px"><h1 style="font-size:1.8rem">Yönetim paneli</h1><button class="btn btn-o sm" id="out">Çıkış</button></div>' +
    ...
    var rs = document.getElementById("rs"); if (rs) rs.onclick = function () { if (confirm("Tüm demo verileri silinsin mi?")) API.sifirla().then(yukle); };
